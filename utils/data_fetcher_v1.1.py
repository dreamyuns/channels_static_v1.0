# utils/data_fetcher_v1.1.py
"""데이터 조회 및 처리 함수 v1.1 - 날짜유형/예약상태 필터 추가"""
...
_query_builder_path = os.path.join(os.path.dirname(__file__), 'query_builder_v1.1.py')
spec = importlib.util.spec_from_file_location("query_builder_v1_1", _query_builder_path)
query_builder_v1_1 = importlib.util.module_from_spec(spec)
sys.modules["query_builder_v1_1"] = query_builder_v1_1
spec.loader.exec_module(query_builder_v1_1)

from query_builder_v1_1 import (  # type: ignore
...
def fetch_summary_stats(start_date, end_date, date_type='전체', order_status='전체'):
    """
    요약 통계 조회
...
def fetch_daily_trend(start_date, end_date, date_type='전체', order_status='전체'):
    """
    일별 추세 데이터 조회
...
def fetch_channel_performance(start_date, end_date, date_type='전체', order_status='전체'):
    """
    채널별 성과 데이터 조회
...
if __name__ == "__main__":
    ...
    # 2. 요약 통계
    print("\n[2. 요약 통계] - 날짜유형: 전체, 예약상태: 전체")
    stats = fetch_summary_stats(start_date, end_date, '전체', '전체')
    for key, value in stats.items():
        print(f"  - {key}: {value:,}")
    
    # 3. 채널별 데이터
    print("\n[3. 채널별 예약 데이터] - 날짜유형: useDate, 예약상태: 확정")
    df = fetch_channel_data(start_date, end_date, None, 'useDate', '확정')
    if not df.empty:
